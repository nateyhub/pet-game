...
if(floor(x) == intersection_top_left_obj.x) {
	//random_set_seed(floor(power((dog_city_obj.x + dog_city_obj.y), 2)))

	show_debug_message("x:" + string(floor(x)) + " " + "x2:" + string(intersection_top_left_obj.x))
...
switch(direction) {
	case 0:
		...
		sprite_index = newcar_x_spr
		image_xscale = 1
		image_yscale = 1
		_direction = "right"
		if (collision_rectangle(x-28,y-18,x+36,y+16,dog_city_obj, false,false)) {
			show_debug_message("Dog is here")
			path_speed = 0
		} else {
			path_speed = temp_speed
	...
	case 90:
		...
		sprite_index = newcar_y_spr
		image_xscale = 1
		image_yscale = 1
		_direction = "up"
		if (collision_rectangle(x-18,y-36,x+18,y+26,dog_city_obj, false,false)) {
			show_debug_message("Dog is here")
			path_speed = 0
		} else {
			path_speed = temp_speed
	...
	case 180:
		...
		image_yscale = 1
		_direction = "left"
		if (collision_rectangle(x-36,y-18,x+28,y+16,dog_city_obj, false,false)) {
			show_debug_message("Dog is here")
			path_speed = 0
		} else {
			path_speed = temp_speed
	...
	case 270:
		...
		image_xscale = 1
		image_yscale= -1
		if (collision_rectangle(x-18,y-26,x+18,y+36,dog_city_obj, false,false)) {
			show_debug_message("Dog is here")
			path_speed = 0
		} else {
			path_speed = temp_speed
